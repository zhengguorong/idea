...
module.exports = app => {
  ...
  class MailService extends app.Service {
    ...
    * send(to, subject, text, html) {
    ...
    // 发送邀请邮件
    * sendInvitation({ developer, title, charge, platform, startDate, endDate, detail }) {
      let developerStr = '';
      ...
      for (let i = 0; i < developer.length; i++) {
        ...
        if (item.email) {
          ...
          const releaseHtml = style + headerHtml + chargeHtml + developerHtml + platformHtml + startDateHtml + endDateHtml + detailHtml + footerHtml;
